...
function writeProxyLog(logger, event, degradeLog = null, affinityLog = null, observationLog = null) {
  ...
  logger(`${fields.join(' ')}${degradeLogFields(degradeLog)}${affinityLogFields(affinityLog)}${observationLogFields(observationLog, affinityLog)}`);
}
...
function buildObservationLog({ req, body, observation, upstreamResponse, observability }) {
  ...
  return {
    ...
    sid: observationSidHash(observationSessionKeyFrom(req, sessionSource)),
