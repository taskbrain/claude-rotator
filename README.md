...
OAuth access token は期限の5分前から自動更新対象になります。保存時に Claude Code の OAuth scope と refresh token の期限を保持し、更新要求にも同じ scope を送ります。access token が数時間で切れること自体は通常ですが、refresh token が有効な間は対話ログインなしで更新されます。同じ refresh token に対する同時更新は1回へ集約し、異なるアカウントのtoken endpoint呼び出しも直列化します。OAuth server が新しい refresh token を返した場合は、使用前に Keychain またはLinuxのcredential fileへ保存します。token endpoint が429を返した場合、providerが明示した `Retry-After` は増幅せず、異常値による永久停止を避ける24時間の安全上限内で尊重します。`Retry-After` が無い場合だけ、refresh token単位で1/2/4/8/15分までbackoffし、15分待機が続いた後は指数を増やさず固定60分の疎通確認へ移ります。1アカウントの待機は他アカウントの更新を停止せず、再リンク後のaccount reloadで古い待機状態を解除します。現在のClaude Codeログインと `accountUuid` が一致する保存アカウントには、Claude Codeが更新した最新credentialとOAuth metadataをミラーします。これにより、後でClaude Codeのログイン先を変えても古いrefresh tokenへ戻りません。provider側で明示的にrevocationされたtokenの期限自体をrotatorから延長することはできません。
...
OAuth access tokens become refresh candidates 5 minutes before expiry. Claude Code OAuth scopes and refresh-token expiry metadata are preserved and the same scopes are sent during refresh. Short-lived access tokens refresh without an interactive login while their refresh token remains valid. Concurrent refreshes for the same token are coalesced, calls for different accounts are serialized, rotated credentials are persisted before use, and the current Claude Code credential and OAuth metadata are mirrored to the matching saved account. An explicit provider `Retry-After` is honored without amplification, up to a 24-hour safety ceiling. When it is absent, repeated 429 responses back off per refresh token through 1/2/4/8/15 minutes, then move to a fixed hourly probe instead of further exponential growth. Reloading accounts after a relink clears the stale credential cooldown. Provider-side revocation and maximum token lifetime remain controlled by the OAuth provider.
